...
public class PhotoComponent extends JComponent implements KeyListener,
		MouseListener, MouseMotionListener {

	private static final long serialVersionUID = 1L;
	private final static String placeholder = "Insert text";
	private ArrayList<Point> currentLine;
	private ArrayList<ArrayList<Point>> linesInFront, linesInBack;
	...
	public void paintComponent(Graphics g) {
		...
		/* Draw a photo, strokes & texts */
		if (image != null) {
			this.updateView();

			if (isFlipped == false) {
				/* Draw photo */
				g2d.drawImage(image, left, top, right, bottom, 0, 0, imgWidth,
						imgHeight, null);
				/* Draw stroke */
	...
	public void writeText(Graphics2D g2d) {
		...
		for (Text aText : texts) {
			...
			if (aText.text.length() > 0) {
				...
				LineBreakMeasurer measurer = new LineBreakMeasurer(paragraph,
						frc);
				if (breakWidth > 0) {
					while (measurer.getPosition() < paragraph.getEndIndex()
						...
						drawPosY += textLayout.getDescent()
	...
	private void resetTextInput() {
		if (currentText != null) {
			if (currentText.text == placeholder) {
				textsInBack.remove(currentText);
				textsInFront.remove(currentText);
			}
		}
		currentText = null;
	}
	...
	public void mouseClicked(MouseEvent e) {
		if (image == null)
			return;
		/* Convert points to stock */
		Point physicPoint = e.getPoint();
		Point viewPoint = physicToLogicPoint(physicPoint);

		if (physicPoint.x > left && physicPoint.y > top
				&& physicPoint.x < right && physicPoint.y < bottom) {
			/* flipping */
